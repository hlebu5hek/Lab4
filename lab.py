...
if min_ > sum_:
    print("Minimal in odd columns ({:1}) is greater than Summ of odd rows ({:2})\n".format(min_, sum_))

    for i in range(m):
        for j in range(m):
            c[i][j], b[i][m-j-1] = b[i][m-j-1], c[i][j]
else:
    print("Minimal in odd columns ({:1}) is less than Summ of odd rows ({:2})\n".format(min_, sum_))
...
if deta > sum_: #A-1*AT – K * F-1
    print("Det A ({:1}) is greater than Summ main diagonal ({:2})\n".format(int(deta), sum_))
    ...
    out_ = np.add(a_arr_inv_mul_trans, (-1)*f_arr_inv)
    print("A-1 * AT – K * F-1 : ")
    printList(list(out_))

else: #(AТ +G-FТ)*K
    print("Det A ({:1}) is less than Summ main diagonal ({:2})\n".format(int(deta), sum_))
    print("AT : ")
